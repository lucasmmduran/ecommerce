$(document).ready(function(){
    var  activeTab= $('input[name="submited_tab"]').val().trim();
    ...
    $('#ybc_submit_export').click(function(){
        if(!$('#ybc_submit_export').hasClass('active') && confirm($('#ybc_export_warning_msg').text()))
        {
            ...
            $.ajax({
                ...
                type: 'post',
                dataType: 'json',
                data: 'export_data=1&IMPORT_EXPORT_IMAGE_GREY='+$('input[name="IMPORT_EXPORT_IMAGE_GREY"]:checked').val(),
                success: function(json)
                {
    ...
    $(document).on('change','input[name="YBC_TC_CACHE_CSS"]',function(){
        
        if(parseInt($(this).val())==1)
            $('.ybc_tc_clear_cache').addClass('active');
        else
            $('.ybc_tc_clear_cache').removeClass('active');
    });    
    if(parseInt($('input[name="YBC_TC_CACHE_CSS"]:checked').val())==1)
        $('.ybc_tc_clear_cache').addClass('active');
    else
        $('.ybc_tc_clear_cache').removeClass('active');
    $(document).on('click','.ybc_tc_clear_cache',function(){
        if(!$(this).hasClass('clearing'))
        {
            ...
            $.ajax({
                ...
                success: function(json)
                {
                      $('.ybc_tc_clear_cache').removeClass('clearing');
                      $('.ybc-update-success-msg').remove();
                      $('.ybc-tc-loading').after('<div class="ybc-update-success-msg" style="display: none;">'+json.success+'</div>');
                      $('.ybc-update-success-msg').fadeIn().delay(3000).fadeOut();
                },
                ...
                {
                    $('.ybc_tc_clear_cache').removeClass('clearing');
                    alert('Something went wrong! Please check your internet connection and make sure you have logged in');
                }
            });
        }
        return false;
