...
class Ybc_themeconfig extends Module
{
    ...
    public function __construct()
	{
       ...
       $this->gfonts = array(
            ...
            'https://fonts.googleapis.com/css?family=Montserrat:200,300,400,500,700,800,900&amp;subset=latin-ext,vietnamese',            
       );
       $this->exTabs = array(
       ...
       $this->configs = array(
            ...
            'YBC_TC_STORE_LINK' => array(
                'label' => $this->l('Show "Store" link'),
                'type' => 'switch',
                'default' => 1,
                'body_class' => true, 
                'group' => 'ybc_tab_header',
            ...
            'YBC_TC_TRACK_ORDER' => array(
                'label' => $this->l('Show "Track Order" link'),
                'type' => 'switch',
                'default' => 1,
                'body_class' => true, 
                'group' => 'ybc_tab_header',
				...
				'options' => array(
        			 'query' => array(                             
                            ...
                            array(
                                ...
                                'slides' => array(3,7),
                                'widget' => array(14,53,54,70,71,72,73,74,75,76),
                                'menus' => array(1,2,3,4,5,6,7),                                            
                            ),
                            array(
            ...
            'YBC_TC_COPYRIGHT_TEXT_EXTRA' => array(                
                'label' => $this->l('Opening hours'),				
                'lang' => true,
                'type' => 'textarea',
                'group' => 'ybc_tab_footer',                
                'default' => '<h4>Opening hours:</h4><br>
                                Monday to friday<span> ...................</span> 7:00  to   16:00<br>
                                Saturday<span> ................................</span> 8:00 to 18:00<br>
                                Sunday<span> ..................................</span> 6:00 to  20:00
                            ',
            ), 
            ...
            'YBC_TC_COPYRIGHT_TEXT' => array(                
                ...
                'type' => 'textarea',
                'group' => 'ybc_tab_footer',                
                'default' => 'Copyright 2017 <a href="#">Fresh Food Co., LTD</a>. All rights reserved.',
            ), 
            ...
            'YBC_TC_ENABLE_SERVICES_MOBILE' => array(                
                'label' => $this->l("Enable Services block in mobile"),
                'type' => 'switch',
                'default' => 0,
                'group' => 'ybc_tab_home',
            ),
            'YBC_TC_ENABLE_SEARCHED' => array(                
                'label' => $this->l("Enable 'Most searched' in mobile"),
                'type' => 'switch',
                'default' => 0,
                'group' => 'ybc_tab_home',
            ),
            ...
            'YBC_TC_FONT1_DATA' => array(                
                ...
                'separator' => true,   
                'data_type' => 'font',     
                'default' => 'https://fonts.googleapis.com/css?family=PT+Sans:400,700',
                'validate' => 'isString',                              
            ),              
            ...
            'YBC_TC_FONT2_DATA' => array(                
                ...
                'separator' => true,   
                'data_type' => 'font',  
                'default' => 'https://fonts.googleapis.com/css?family=Montserrat:200,300,400,500,700,800,900&amp;subset=latin-ext,vietnamese',
                'validate' => 'isString',               
            ),              
            ...
            'YBC_TC_LABEL_CART' => array(                
                'label' => $this->l('Label for button add cart'),
                'type' => 'text',                
                'group' => 'ybc_tab_product',    
                'default' => 'Order now',   
                'lang' => true,         
           ...
           'IMPORT_EXPORT_IMAGE_GREY' => array(
               'group' => 'ybc_tab_import',
               'label' => $this->l('Grey out image'),
               'type' => 'switch',
               'default' => 1,
    ...
    public function renderConfig()
    {
                ...
                if(isset($config['type']) && $config['type'] == 'file')
                {
                    if($imageName = Configuration::get($key))
                    {
                        ...
                        if(!isset($config['required']) || (isset($config['required']) && !$config['required']))
                            $confFields['img_del_link'] = $this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name .'&delimage=yes&image='.$key;
                    }
                }
        ...
        $helper->tpl_vars = array(
            ...
            'export_link' => $this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&exportNewsletter=yes',
            'clear_cache_url' => $this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&clearcache=1',
     ...
     private function exportTable($tbls,$savePath,$layout,$mod = false, $greyOutImage=true)
     {
        if(!is_array($tbls))
            $tbls = array($tbls);
        $sql = "";
                            ...
                            foreach($row as $key => $val)
                            {
                                ...
                                {                                    
                                    ...
                                    {   
                                        ...
                                        $this->greyOutImage($images[$tblName]['src'].$val,$images[$tblName]['des'], $greyOutImage);
                                    }
                                }
                                else
                                {
                                    ...
                                    {                                    
                                        ...
                                        $this->greyOutImage($images[$tblName]['src'].$val,$images[$tblName]['mod'], $greyOutImage);
                                    }
                                }
                            }    
                            $sql = trim($sql,',').");\n";
        ...
        if(Tools::isSubmit('export_data'))
        {
            ...
            if($layouts = $configs['YBC_TC_LAYOUT']['options']['query'])
            {
                foreach($layouts as $layout)
                {
                    ...
                    $this->exportTable($tbls,$dataDir.$layoutName.'/sql/widget.sql',$layoutName,false,Tools::getValue('IMPORT_EXPORT_IMAGE_GREY'));
                    if($defaultLayout==$layoutName)
                    {
                        $this->exportTable($tbls,dirname(__FILE__).'/../ybc_widget/data/sql/widget.sql',$layoutName,true,Tools::getValue('IMPORT_EXPORT_IMAGE_GREY'));
                    }                    
                    $tbls = array(
                        array(
                    ...
                    $this->exportTable($tbls,$dataDir.$layoutName.'/sql/widget_lang.sql',$layoutName,false,Tools::getValue('IMPORT_EXPORT_IMAGE_GREY'));
                    if($defaultLayout==$layoutName)
                    {
                        $this->exportTable($tbls,dirname(__FILE__).'/../ybc_widget/data/sql/widget_lang.sql',$layoutName,true,Tools::getValue('IMPORT_EXPORT_IMAGE_GREY'));
                    }
                }
            }
            if(class_exists('Ets_multilayerslider'))
            {
                ...
                $slider->generateArchive(dirname(__FILE__).'/../ets_multilayerslider/data/init.data.zip',Tools::getValue('IMPORT_EXPORT_IMAGE_GREY'));
            }
            if(class_exists('Ets_megamenu'))
            {
                $menu = new Ets_megamenu();
                $menu->generateArchive(dirname(__FILE__).'/../ets_megamenu/data/init.data.zip',Tools::getValue('IMPORT_EXPORT_IMAGE_GREY'));
            }
            if(class_exists('Ybc_blog'))
            {
                $blog = new Ybc_blog();
                // Upgrade
                $blog->generateArchive(dirname(__FILE__).'/../ybc_blog/data/init.data.zip',Tools::getValue('IMPORT_EXPORT_IMAGE_GREY'));
            }
    ...
    public function greyOutImage($src,$descDir, $greyOutImage=true)
    {
        ...
        if(file_exists($descDir.$name))
            @unlink($descDir.$name);
        if(($extension == 'png' || $extension=='jpg') && $greyOutImage)
        {
    ...
    public function renderCss()
    {
        ...
        {
            $css .= 'h1,h2,h3,h4,h5,.ets_mm_megamenu .mm_menus_li > a,h6{font-family: '.$font2.';}';
        }
