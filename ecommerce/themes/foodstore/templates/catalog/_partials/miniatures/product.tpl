...
*}
<article
        class="{if isset($check) && ($check eq true)}col-md-3 col-xs-12{/if} product-miniature js-product-miniature {if isset($tc_config.YBC_TC_FLOAT_CSS3) && $tc_config.YBC_TC_FLOAT_CSS3 == 1} wow zoomIn{/if}"
        data-id-product="{$product.id_product|escape:'html':'UTF-8'}" data-id-product-attribute="{$product.id_product_attribute|escape:'html':'UTF-8'}" itemscope
                ...
                {if $product.show_price}
                    ...
                    {foreach from=$product.flags item=flag}
                        {if $flag.type != 'discount'}
                            <li class="new">
                                <span class="new_content">{$flag.label|escape:'html':'UTF-8'}</span>
                            </li>
                        {/if}
                    {/foreach}
