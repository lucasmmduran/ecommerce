  ...
  <div class="blockcart cart-preview {if $cart.products_count > 0}active{else}inactive{/if}" data-refresh-url="{$refresh_url|escape:'html':'UTF-8'}">
    <a rel="nofollow" href="{$cart_url|escape:'html':'UTF-8'}">
       <i class="fa fa-shopping-bag"></i>
        <span class="hidden-sm-down checkout_title">{l s='My Cart' d='Shop.Theme.Checkout'}</span>
        {*<span class="checkout_total">
             {$cart.products_count|escape:'html':'UTF-8'}
            {if $cart.products_count <= 1}
            {l s='Item' d='Shop.Theme.Actions'}
            {else}
            {l s='Items' d='Shop.Theme.Actions'}{/if} -  {$cart.totals.total.value|escape:'html':'UTF-8'}
        </span>*}
