...
$(document).ready(function(){   
   ...
   $(document).on('click','.input-color',function(e) {
    ...
    if($('#_nav_cart').length != 0)
    {
      $('#_nav_cart').html($('#_desktop_cart').html());
      $('#_desktop_cart').html('');
    }
    ...
    $(document).on('click','.user-info-account > span, .toogle_nav_button',function(e){
        if ($(window).width() <= 767)
        $(this).next().toggleClass('active');
        
   });
   $('.search_close').on('click',function(e){
       $('.search_icon_toogle').removeClass('open');
   ...
   $('body:not(#index) .category-top-menu-pos').addClass('not_index');
    ...
    if ( $('.ybc-widget-ybc-custom-5').length != ''){
...
{
	elementClick1 = '#search_widget .search_icon_toogle';
	elementSlide1 =  '.search_form';
	activeClass = 'open';

	$(elementClick1).on('click', function(e){
    		e.stopPropagation();
    		var subUl = $(this).next(elementSlide1);
    		if(subUl.hasClass('ybc_hide'))
    		{
    			$(this).addClass(activeClass);
                $(elementSlide1).addClass(activeClass);
    		}
    		else
    		{
    			$(this).removeClass(activeClass);
                $(elementSlide1).removeClass(activeClass);
                $(this).parent().parent().removeClass(activeClass);
                
    		}
            if ($(window).width() >= 768){
        		$(elementClick1).not(this).next(elementSlide1).removeClass(activeClass);
        		$(elementClick1).not(this).removeClass(activeClass);
                $(elementClick1).not(this).parent().parent().removeClass(activeClass);
        		e.preventDefault();
            }
	});
	...
	$(document).on('click', function(e){
	    if ($(window).width() >= 768){
            ...
            $(elementSlide1).removeClass(activeClass);
    		$(elementClick1).removeClass(activeClass);
            $(elementClick1).parent().parent().removeClass(activeClass);
            $(elementClick1).parent().find('.search_block_top_close').removeClass(activeClass);
        }
	});
