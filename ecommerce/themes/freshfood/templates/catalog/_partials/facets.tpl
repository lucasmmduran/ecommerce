...
*}
  <div id="search_filters">
    <h4 class="text-uppercase h6 hidden-xs-down">{l s='Filter By' d='Shop.Theme.Actions'}</h4>
    <div id="_desktop_search_filters_clear_all" class="hidden-xs-down clear-all-wrapper">
    ...
    {foreach from=$facets item="facet"}
      {if $facet.displayed}
        <section class="facet">
          <h2 class="h6 facet-title hidden-xs-down"> {$facet.label|escape:'html':'UTF-8'}</h2>
          {assign var=_expand_id value=10|mt_rand:100000}
          {assign var=_collapse value=true}
          {foreach from=$facet.filters item="filter"}
            {if $filter.active}{assign var=_collapse value=false}{/if}
          {/foreach}
          <div class="title hidden-sm-up" data-target="#facet_{$_expand_id|escape:'html':'UTF-8'}" data-toggle="collapse"{if !$_collapse} aria-expanded="true"{/if}>
